...
gGlobal.checkIfItIsGloballySelected = function(nodeName, drawto_) {
	return (ctxInGlobal() && gGlobal.contexts[drawto_].currentlySelected == nodeName);
}
...
function removeFromGlobalCtxMap() {
	...
	if(drawto !== "" && gGlobal.contexts[drawto] !== undefined) {
		...
		if(gGlobal.checkIfItIsGloballySelected(nodeCTX.name, drawto)) {
