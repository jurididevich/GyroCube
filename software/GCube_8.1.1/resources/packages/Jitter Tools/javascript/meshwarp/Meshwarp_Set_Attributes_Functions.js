...
function output_mesh_matrix() {
	gMesh.outputPositionMatrix();
}

function mesh_matrix(matName) {
	gMesh.setNewPositionMatrix(matName);
}
...
function buildSaveDict() {
	...
	saveDict.replace("apply_mask", apply_mask);
	saveDict.replace("invert_mask", invert_mask);
...
function loadFromDict(saveDict) {
	...
	apply_mask = saveDict.get("apply_mask");
	invert_mask = saveDict.get("invert_mask");
