...
Mesh.prototype.mouseClickedRoutine = function(mouseState, oldMouseState_) {
    ...
    if (mouseClicked > 0) {
        ...
        if (this.checkIfMouseIsInsideMesh(mouseWorld) == this.ID) {
            ...
            if (mask_mode && gGlobal.checkIfItIsGloballySelected(nodeCTX.name, drawto))
            {   
                if (mouseClicked == 2 && gShiftPressed)
                {       
                    debug(DEBUG.MASK, "drawto "+drawto + " - create mask")
        ...
        if (gGlobal.checkIfItIsGloballySelected(nodeCTX.name, drawto) && mask_mode)
        {
            if (mouseClicked == 2 && gShiftPressed)
            {   
                this.deleteMaskMesh();
    ...
    else if (mask_mode && gGlobal.checkIfItIsGloballySelected(nodeCTX.name, drawto))
    {
